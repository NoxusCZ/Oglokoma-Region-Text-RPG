...
void bojSeDvemaMonstry(Postava &hrac) {
    ...
    while (hrac.hp > 0 && (m1.hp > 0 || m2.hp > 0)) {
        cout << "\nTvuj stav: HP " << hrac.hp << "/" << hrac.maxHp << "  Mana: " << hrac.mana << "/" << hrac.maxMana << endl;
        if (m1.hp > 0)
        cout << "1) " << m1.jmeno << " (HP: " << m1.hp << ")" << endl;
        if (m2.hp > 0)
        cout << "2) " << m2.jmeno << " (HP: " << m2.hp << ")" << endl;
        if (hrac.kouzla > 0)
        cout << "3) Pouzit kouzlo" << endl;
        cout << "Tvoje volba: ";
        int volba;
        cin >> volba;
        ...
        if (m1.hp > 0) {
            if (zamr1) {
            cout << m1.jmeno << " je zamrazen a neutoci!" << endl; zamr1 = 0; }
            else {
            hrac.hp -= m1.utok; cout << m1.jmeno << " te zasahuje za " << m1.utok << " dmg!" << endl; }
        }
        if (m2.hp > 0) {
            if (zamr2) {
            cout << m2.jmeno << " je zamrazen a neutoci!" << endl; zamr2 = 0;
            }
            else {
        hrac.hp -= m2.utok; cout << m2.jmeno << " te zasahuje za " << m2.utok << " dmg!" << endl;
        }
        }
    }
...
void bojSGangem(Postava &hrac) {
    ...
    while (hrac.hp > 0 && pocetZivych > 0) {
        ...
        for (int i = 0; i < 4; i++) {
            if (gang[i].hp > 0) {
                if (zamr[i]) {
                    cout << gang[i].jmeno << " je zamrazen!" << endl; zamr[i] = 0;
                }
                else {
            hrac.hp -= gang[i].utok; cout << gang[i].jmeno << " te seka za " << gang[i].utok << " dmg!" << endl;
            }
            }
        }
    }
